...
def _load_tensor(fp, tensor_name: str, dtype_name: str, data_offsets: Tuple[int, int], shape: List[int]) -> torch.Tensor:
    ...
    fp.seek(data_offsets[0], os.SEEK_SET)
...
def load_safetensors(path: PathLike):
    with open(path, "rb") as f:
        ...
        for key, tensor_info in header.items():
            ...
            tensor = _load_tensor(f, key, tensor_info['dtype'], tensor_info['data_offsets'], tensor_info['shape'])
